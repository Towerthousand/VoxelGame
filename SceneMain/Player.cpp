...
Player::Player(World &world) : Camera(world) {
    vel = vec3f(0, 0, 0);
    selectedID = 1;
    frustumPlanes = std::vector<std::vector<vec3f> >
            (6,std::vector<vec3f>
             (4,vec3f(0,0,0)));
}
...
bool Player::insideFrustum(vec3f position, float radius) {
    float distance,D;
    bool result = true;
    for(int i=0; i < 6; i++) {
        ...
        distance = dot(n,position) + D;  //Solve the equation using the player's pos instead of a point in the plane.
