...
float3Data Function3DSimplex::getFloat3Data(int ox, int oy, int oz, int sx, int sy, int sz) { //world coords
	...
	for(int localX = 0; localX < sx; ++localX)
		for(int localY = 0; localY < sy; ++localY)
			for(int localZ = 0; localZ < sz; ++localZ) {
				const float x = (ox+localX)/scale;
				const float y = (oy+localY)/scale;
				const float z = (oz+localZ)/scale;
				...
				//scale the result
				result[localX][localY][localZ] = min + (max-min) * ((16.0*(n0 + n1 + n2 + n3) + 0.5));
			}
	return result;
}

int Function3DSimplex::fastfloor( const float x ) { return x > 0 ? (int) x : (int) x - 1; }
