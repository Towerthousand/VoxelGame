...
void Chunk::setCubeID(int x, int y, int z, int id) {
    if (y < 0 || y >= CHUNKHEIGHT) //over the skylimit, under bedrock
        return;
    else if (x >= CHUNKWIDTH || z >= CHUNKWIDTH || x < 0 || z < 0)//outside of this chunk
        parentWorld.setCubeIDAbs(x+(XPOS*CHUNKWIDTH),y,z+(ZPOS*CHUNKWIDTH),id);
    else //inside this chunk
        cubes[x][y][z].id = id;
    markedForRedraw = true;
}

int Chunk::getCubeID(int x, int y, int z) const {
    if (y < 0 || y >= CHUNKHEIGHT) //over the skylimit, under bedrock
        return 0;
    if (x >= CHUNKWIDTH || z >= CHUNKWIDTH || x < 0 || z < 0)//outside of this chunk
        return parentWorld.getCubeAbsID(x+(XPOS*CHUNKWIDTH),y,z+(ZPOS*CHUNKWIDTH));
    return cubes[x][y][z].id; //inside current chunk
}
...
void Chunk::pushCubeToArray(int x,int y, int z,int cubeID) {
    ...
    if(getCubeID(x,y,z+1) == 0) { // front face
    ...
    if(getCubeID(x,y,z-1) == 0) { // back face
    ...
    if(getCubeID(x+1,y,z) == 0) { // left face
    ...
    if(getCubeID(x-1,y,z) == 0) { // right face
    ...
    if(getCubeID(x,y-1,z) == 0) { // bottom face
    ...
    if(getCubeID(x,y+1,z) == 0) { // top face
...
void Chunk::updateGrass(float deltaTime) { //only to be called by main update()
    if (grassTimer >= 0.01) {
        ...
        if (getCubeID(x,y+1,z) != 0 && getCubeID(x,y,z) == 3) {
            setCubeID(x,y,z,1);
            markedForRedraw = true;
        }
    }
